          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Link to="/pos" style={{ textDecoration: 'none' }}>
              <ListItemButton sx={{ borderRadius: 3, bgcolor: 'primary.main', color: 'white', '&:hover': { bgcolor: 'primary.dark' }, px: { xs: 2, md: 3 }, py: 1, boxShadow: `0 4px 16px -6px ${theme.palette.primary.main}` }}>
                ...
                {!isMobile && <Typography variant="button" fontWeight={800} sx={{ letterSpacing: 0.5 }}>TERMINAL</Typography>}
              </ListItemButton>
            </Link>
          </Box>
        </Toolbar>
                ...
                {item.children ? (
                  <>
                    <ListItem disablePadding sx={{ mb: 0.5 }}>
                      ...
                      >
                        <ListItemIcon sx={{ color: openMenus[item.title] ? 'primary.main' : 'text.secondary', minWidth: 44 }}>
                          {item.icon}
                        </ListItemIcon>
                        <ListItemText primary={item.title} primaryTypographyProps={{ fontWeight: 700, fontSize: '0.95rem', letterSpacing: -0.2 }} />
                        {openMenus[item.title] ? <ExpandLess sx={{ opacity: 0.5 }} /> : <ExpandMore sx={{ opacity: 0.5 }} />}
                      </ListItemButton>
                    </ListItem>
        ...
        <Box sx={{ p: 2.5, borderTop: `1px solid ${alpha(theme.palette.divider, 0.05)}`, bgcolor: alpha(theme.palette.action.hover, 0.02) }}>
           ...
           >
              ...
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography variant="subtitle2" fontWeight={800} noWrap sx={{ letterSpacing: -0.2 }}>{user?.email?.split('@')[0] || 'Admin User'}</Typography>
